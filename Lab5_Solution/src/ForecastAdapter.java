...
public class ForecastAdapter implements ListViewAdapter {
	...
	private ImageIcon getScaledImageIcon(int w, int h, String img) {
		ImageIcon icon1 = new ImageIcon("../Lab5_Images/" + img + ".png");
	...
	public List<View> getViews() {
		...
		views.add(new View(Layout.getLabel1(0), "Manchester NH"));
		int n = 1;
		for (int i = 0; i < data.size(); i++) {
			...
			if (i == 0) {
				for (int j = 0; j < 5; j++) {
					...
					case 1: icon = getScaledImageIcon(label.getWidth(), label.getHeight(), fc.getIcon()); break;
				...
				for (int j = 0; j < 5; j++) {
					...
					ImageIcon icon = null;
					switch (j) {
					case 0: icon = getScaledImageIcon(label.getWidth(), label.getHeight(), fc.getIcon()); break;
